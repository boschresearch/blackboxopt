...
#     - factor the objective-agnostic KDE machinery into an abstract base sampler
...
class KDESampler(StagedIterationConfigurationSampler, abc.ABC):
    """Abstract, objective-count-agnostic BOHB kernel density estimator sampler.
    ...
    def __init__(
        ...
        self.losses: Dict[float, List[Union[float, np.ndarray]]] = dict()
    ...
    def digest_evaluation(self, evaluation: Evaluation):
        ...
        """
        loss = self._evaluation_to_loss(evaluation)
        ...
        if self._count_finite_losses(self.losses[fidelity]) <= (
            self.min_samples_in_model - 1
        ):
            n_runs_finite_loss = self._count_finite_losses(self.losses[fidelity])
            self.logger.debug(
        ...
        idx_good, idx_bad = self._good_bad_split(train_losses, n_good, n_bad)

        train_data_good = impute_conditional_data(
            train_configs[idx_good], self.vartypes, rng=self._rng
        )
        train_data_bad = impute_conditional_data(
            train_configs[idx_bad], self.vartypes, rng=self._rng
        )
