...
class Datum:
    ...
    status: str
    losses: Optional[np.ndarray] = field(default=None)
...
class StagedIteration:
    def __init__(
        ...
        Args:
            ...
            config_promotion_function: Function that decides which configurations are
                ...
                the signature.
            objectives: The objectives of the optimization. Single-objective callers
                must wrap their objective in a length-one sequence.
    ...
    def digest_evaluation(
        ...
        d.losses = np.asarray(
            get_loss_vector(
                known_objectives=self._objectives,
    ...
    def _progress_to_next_stage(self):
        ...
        # filter out crashed configurations (any non-finite objective value)
        data = [
            d
            for d in self.evaluation_data[self.current_stage]
            if d.losses is not None and np.all(np.isfinite(d.losses))
        ]
